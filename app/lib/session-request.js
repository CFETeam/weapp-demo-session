...
const buildHeader = co.wrap(function *() {
    if (currentCode) {
        return { [constants.WX_CODE]: currentCode };
    } else {
        return pendingHeader = pendingHeader || co(function *() {
            ...
            return {
                [constants.WX_CODE]: currentCode,
                [constants.WX_RAW_DATA]: encodeURIComponent(rawData),
                [constants.WX_SIGNATURE]: signature,
            };
        });
    }
});
...
function requestWithSession(options = {}) {
    ...
    const wrapRequest = co.wrap(function * () {
        ...
        if (tryTimes++ > constants.MAX_RETRY_TIMES) {
        ...
        return wx.request(Object.assign({}, options, {
            ...
            success({ data }) {
                if (data[constants.WX_SESSION_MAGIC_ID]) {
                    const error = data.error;

                    switch (data.reason) {
                    case 'ERR_SESSION_EXPIRED':
                    case 'ERR_SESSION_KEY_EXCHANGE_FAILED':
                        currentCode = null;
                        return wrapRequest();

                    case 'ERR_UNTRUSTED_RAW_DATA':
                    case 'ERR_JSCODE_NOT_PASSED':
                    default:
